...
[![Travis build status](https://travis-ci.com/muschellij2/MriCloudR.svg?branch=master)](https://travis-ci.com/muschellij2/MriCloudR)
  [![AppVeyor build status](https://ci.appveyor.com/api/projects/status/github/muschellij2/MriCloudR?branch=master&svg=true)](https://ci.appveyor.com/project/muschellij2/MriCloudR)
...
Because `MriCloudR` currently only works with email/password combinations, we have to store our passwords.  The `keyringr` package has a [great vignette](https://cran.r-project.org/web/packages/keyringr/vignettes/Avoiding_plain_text_passwords_in_R_with_keyringr.html) to show how to store your password by the password manaager for your operating system.  In our example, we have named the keychain (OS X) entry as `mricloudr`.
